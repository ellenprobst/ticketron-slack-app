...
async function downloadSlackFile(file, token) {
  ...
  if (!resp.ok) {
    console.log('\x1b[33m%s\x1b[0m', `[downloadImages] Failed to download file ${file.name}: ${resp.status}`);
    return null;
  }

  const contentType = resp.headers.get('content-type') || '';
  if (!contentType.startsWith('image/')) {
    console.log('\x1b[33m%s\x1b[0m', `[downloadImages] Skipping ${file.name}: expected image, got ${contentType}`);
    return null;
  }
...
export async function downloadImages({ threadMessages, event, token }) {
  const allFiles = [...threadMessages.flatMap((msg) => msg.files || []), ...(event.files || [])];
  const imageFiles = allFiles.filter((f) => SUPPORTED_IMAGE_TYPES.has(f.mimetype));
  ...
  for (const file of imageFiles) {
    try {
      ...
      console.log('\x1b[33m%s\x1b[0m', `[downloadImages] Error downloading file ${file.name}: ${err.message}`);
    }
  }
