 ...
 */
export async function runAgentWithMessage({ channel, thread_ts, user, text, client, team, images }) {
  try {
    console.log(
      '\x1b[36m%s\x1b[0m',
    ...
    if (isFreshSession) {
      try {
        ...
        console.log(
          '\x1b[36m%s\x1b[0m',
          `[runAgentWithMessage] Injected Jira config: instance=${instance} project=${project} (channel="${channelName}")`,
        );
      } catch (e) {
        console.log(
          '\x1b[33m%s\x1b[0m',
          `[runAgentWithMessage] Could not resolve channel name (${e.data?.error ?? e.message}), skipping Jira context`,
        );
      }
    }
    ...
    let fullResponse = '';
    console.log('\x1b[35m%s\x1b[0m', '[runAgentWithMessage] Streaming agent response…');
    ...
    try {
      ...
      console.log('\x1b[33m%s\x1b[0m', '[runAgentWithMessage] Response is not valid JSON, treating as chat');
    ...
    if (ticket) {
      console.log('\x1b[35m%s\x1b[0m', `[runAgentWithMessage] Ticket draft: ${ticket.title}`);
    }
    ...
    console.log('\x1b[32m%s\x1b[0m', `[runAgentWithMessage] Done — stage: ${stage}`);
  } catch (error) {
    console.log('\x1b[31m%s\x1b[0m', `[runAgentWithMessage] ERROR: ${error.message}`);
    console.error(error);
...
export const appMentionCallback = async ({ event, client, logger, say }) => {
  try {
    ...
    console.log('\x1b[90m%s\x1b[0m', `[appMention] thread_ts=${thread_ts} (isThread=${!!event.thread_ts})`);
    ...
    const isFreshSession = session.events.length === 0;
    console.log('\x1b[33m%s\x1b[0m', `[appMention] Session fresh=${isFreshSession} (events=${session.events.length})`);
    ...
    if (isFreshSession && event.thread_ts) {
      ...
      console.log('\x1b[35m%s\x1b[0m', `[appMention] Fetched ${threadMessages.length} prior thread messages`);
    }
